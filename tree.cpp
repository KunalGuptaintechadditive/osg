...
osg::ref_ptr<osg::Geode> GraphicsMakeLine(double* startpoint, vector<double*> endpoints)
{
	// Create a Geode to hold our geometry
	osg::ref_ptr<osg::Geode> geode = new osg::Geode;
	geode->setName("Geode2DLine");

	// Create a Geometry object to define the line
	osg::ref_ptr<osg::Geometry> geometry = new osg::Geometry;

	// Create an array to hold the line's vertex data (X, Y, Z)
	osg::ref_ptr<osg::Vec3dArray> vertices = new osg::Vec3dArray;

	// Add the start point to the vertices array
	vertices->push_back(osg::Vec3d(startpoint[0], startpoint[1], startpoint[2]));

	// Push all the endpoints into the vertices array
	for (auto i = 0; i < endpoints.size(); i++) {
		vertices->push_back(osg::Vec3d(endpoints[i][0], endpoints[i][1], endpoints[i][2]));
	}

	// Set the vertex array for the geometry
	geometry->setVertexArray(vertices.get());

	// Connect the start point to each endpoint using indices
	for (unsigned int i = 1; i <= endpoints.size(); ++i) {
		osg::ref_ptr<osg::DrawElementsUInt> drawElements = new osg::DrawElementsUInt(osg::PrimitiveSet::LINES, 2);
		drawElements->push_back(0); // Start point index
		drawElements->push_back(i); // Endpoint index
	...
	// Disable lighting to ensure the color is not affected
...
// 
	...
	*/
	...
	for (int i = 0; i < bottom.size(); i++) {
		...
		double k[3] = { cord[0],cord[1],cord[2] };
		double* linep = k;
		...
		insert.push_back(ver1);
		insert.push_back(ver2);
		insert.push_back(ver3);
		insert.push_back(ver4);
		group->addChild(GraphicsMakeLine(linep, insert));
	...
	for (int i = 0; i < bottom.size(); i++) {
		...
		if (bottom_visited.find(bottom[i]) == bottom_visited.end()) {
			...
			*/
			...
			group->addChild(GraphicsMakeLine(joincenter, insert));
	...
	while (++depth <= level) {
		...
		for (int i = 0; i < grppoints.size(); i++) {
			...
			if (grpvisited.find(arrfind) == grpvisited.end()) {
				...
				group->addChild(GraphicsMakeLine(joincenter, joinee));
				...
				//	Commonfun::DoubleArray arrinsert = { {joinee[i][0], joinee[i][1], joinee[i][2]} };
	...
	for (int i = 0; i < grppoints.size(); i++) {
		...
		if (node[1] > bottom_y) {
			...
			insert.push_back(connect);
			//vtkIdType lineIndices[2] = { 0, 1 }; // Connect Point 4 to Point 0
			//cells->InsertNextCell(2, lineIndices);


			group->addChild(GraphicsMakeLine(nnode, insert));
